"""
AI MINI THERAPIST
Main application using trained FER2013 model
"""
...
# Try to import TensorFlow for trained model
try:
    import tensorflow as tf
    TENSORFLOW_AVAILABLE = True
    print("TensorFlow available - Trained model support enabled")
except:
...
class AIMiniTherapist:
    ...
    def setup_directories(self):
        ...
        # Create log file
        self.activity_log_file = "data/ai_mini_therapist_log.csv"
        if not os.path.exists(self.activity_log_file):
            with open(self.activity_log_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Timestamp', 'Window_Title', 'Application', 'Website', 'Tab', 'Duration', 'Emotion', 'Stress', 'Confidence'])
    ...
    def load_trained_model(self):
        ...
        if TENSORFLOW_AVAILABLE and os.path.exists(model_path):
            try:
                self.trained_model = tf.keras.models.load_model(model_path, compile=False)
                self.model_available = True
                print("Trained FER2013 model loaded successfully!")
            ...
            if not TENSORFLOW_AVAILABLE:
                ...
                print("Trained model not found - Run train_fer2013.py first")
            self.model_available = False
    
    def setup_gui(self):
        """Setup AI Mini Therapist GUI"""
        self.root = tk.Tk()
        ...
        # Model Status
        model_status = "Trained Model: ACTIVE" if self.model_available else "Trained Model: SIMULATION"
        model_color = '#4CAF50' if self.model_available else '#ff9800'
        model_label = tk.Label(self.root, text=model_status, 
                           font=("Arial", 14, "bold"), fg=model_color, bg='#ffffff')
        ...
        # Camera section
        camera_frame = tk.LabelFrame(left_frame, text="AI Camera Monitoring", 
                                   font=("Arial", 14, "bold"), fg='#333333', bg='#ffffff')
        ...
        # Add welcome message
        mode = "Trained Model" if self.model_available else "Simulation"
        self.add_chat_message("AI", f"Welcome! AI Mini Therapist ready. Mode: {mode}. Click Start Session to begin.")
    ...
    def start_session(self):
        """Start session with trained model"""
        try:
            ...
            if self.camera_active:
                self.camera_thread = threading.Thread(target=self.ai_camera_monitoring, daemon=True)
                self.camera_thread.start()
            else:
                self.camera_thread = threading.Thread(target=self.ai_emotion_simulation, daemon=True)
                self.camera_thread.start()
            
            self.activity_thread = threading.Thread(target=self.ai_activity_monitoring, daemon=True)
            self.activity_thread.start()
            
            self.session_timer_thread = threading.Thread(target=self.session_timer, daemon=True)
            self.session_timer_thread.start()
            
            mode = "Trained Model" if self.model_available and self.camera_active else "Simulation"
    ...
    def ai_camera_monitoring(self):
        """AI camera monitoring with trained model"""
        try:
            ...
            while self.running and self.camera_active:
                ...
                for (x, y, w, h) in faces:
                    ...
                    # Use trained model
                    if self.model_available and self.trained_model:
                        try:
                            prediction = self.trained_model.predict(face_input, verbose=0)
                ...
                if frame_count % 10 == 0:
                    self.root.after(0, self.update_ai_camera_display, frame, confidence)
                
                time.sleep(0.05)  # 20 FPS
                
        except Exception as e:
            print(f"AI camera monitoring error: {e}")
    ...
    def ai_emotion_simulation(self):
        ...
        while self.running:
            ...
            if random.random() < 0.25:  # 25% chance of emotion change
                ...
                self.root.after(0, self.update_ai_simulation_display, confidence)
            
            time.sleep(2.5)  # Update every 2.5 seconds
    
    def ai_activity_monitoring(self):
        """AI activity monitoring"""
        activities = [
            ("Chrome - AI Research Paper", "chrome.exe", "arXiv", "AI Research"),
            ("VS Code - ai_mini_therapist.py", "Code.exe", "Desktop Application", "AI Therapist Code"),
            ...
            ("Excel - AI Statistics.xlsx", "EXCEL.EXE", "Desktop Application", "AI Stats")
        ]
        
        activity_index = 0
        
        while self.session_active:
            # Cycle through AI-related activities
    ...
    def trigger_desktop_alert(self, stress_level, emotion, confidence=0.0):
        ...
        # Create AI alert window
        self.alert_window = Toplevel(self.root)
        self.alert_window.title("AI Stress Alert")
        ...
        # Alert content
        tk.Label(self.alert_window, text="AI STRESS DETECTED!", 
        ...
        if confidence > 0:
            tk.Label(self.alert_window, text=f"AI Confidence: {confidence:.1%}", 
        ...
        self.add_chat_message("AI ALERT", f"Stress {stress_level}{conf_text} detected! {intervention}")
    ...
    def update_ai_camera_display(self, frame, confidence):
        """Update AI camera display"""
        try:
            ...
            print(f"AI camera display error: {e}")
    
    def update_ai_simulation_display(self, confidence):
        """Update AI simulation display"""
        try:
            self.camera_canvas.delete("all")
            
            # Create AI simulation display
            self.camera_canvas.create_rectangle(50, 50, 330, 230, fill='#e8f5e8', outline='#d32f2f', width=2)
            self.camera_canvas.create_text(190, 80, text="AI SIMULATION MODE", 
            ...
            self.camera_canvas.create_text(190, 160, text=f"Confidence: {confidence:.1%}", 
                                     fill="black", font=("Arial", 11))
            self.camera_canvas.create_text(190, 185, text="AI Mini Therapist", 
                                     fill="darkgreen", font=("Arial", 10, "italic"))
            
        except Exception as e:
            print(f"AI simulation display error: {e}")
    ...
    def show_statistics(self):
        ...
        try:
            ...
            self.activity_display.insert(tk.END, "=== AI APPLICATION USAGE ===\n")
    ...
    def update_display(self):
        ...
        try:
            ...
            if self.last_confidence > 0:
                self.confidence_label.config(text=f"Conf: {self.last_confidence:.1%}")
    ...
    def generate_response(self, message):
        ...
        try:
            ...
            if any(word in message_lower for word in ['stress', 'stressed', 'anxious']):
            ...
            elif any(word in message_lower for word in ['fer2013', 'fer', 'dataset', 'train']):
                mode = "Real FER2013 detection" if self.model_available else "Simulation mode"
                return f"Using {mode}. Run 'train_fer2013.py' to train on FER2013 dataset."
                ...
                return f"AI session remaining: {minutes} minutes"
                ...
                return "I'm your AI Mini Therapist! I monitor emotions using trained models and provide stress management."
            elif any(word in message_lower for word in ['camera', 'opencv']):
                mode = "Real Camera" if self.camera_active else "Simulation"
                return f"Camera mode: {mode}. AI emotion detection active."
            elif any(word in message_lower for word in ['thank', 'thanks']):
                return "You're welcome! I'm here to help with stress management using AI."
            else:
                return f"AI monitoring your stress level: {self.stress_score}. Ask me about training or features!"
                
        except Exception as e:
            print(f"Response generation error: {e}")
            return "I'm here to help with your AI session."
        ...
        try:
            report = {
                'session_info': {
                    ...
                    'model_used': 'Trained FER2013' if self.model_available else 'Simulation',
                ...
                'ai_info': {
                    'dataset': 'FER2013',
                    'emotions': ['Angry', 'Disgust', 'Fear', 'Happy', 'Sad', 'Surprise', 'Neutral'],
                    'trained_model': self.model_available,
                    'confidence_tracking': True
                }
            }
            
            report_file = f"exports/ai_mini_therapist_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
